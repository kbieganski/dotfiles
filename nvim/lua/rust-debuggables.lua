...
function M.debuggables()
  ...
  if co then
    return coroutine.create(function()
      rt.utils.request(0, "experimental/runnables", get_params(), function(_, result)
        if result == nil then coroutine.resume(co, nil) end
        ...
        vim.ui.select(
          ...
          function(_, choice)
            coroutine.resume(co, choice and result[choice].args or nil)
          end
        )
      end)
    end)
